...
import { getOrdensDeServico, createOrdemDeServico, type OrdemDeServico, type CreateOrdemDeServicoData } from '../api/ordemDeServicoService';
...
const WorkOrdersPage = () => {
  ...
  const handleFormSubmit = async (formData: CreateOrdemDeServicoData) => {
    try {
      ...
      alert('Erro ao abrir Ordem de Serviço: ' + (error.response?.data?.message || 'Erro desconhecido'));
    }
  };
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-8 flex flex-wrap items-center gap-x-8 gap-y-4">
        ...
        <div>
          ...
          <select 
            id="filtro-status"
            value={filtroStatus}
            onChange={(e) => { setFiltroStatus(e.target.value); setPage(1); }} // Reseta a página ao mudar o filtro
        ...
        <div>
          <label htmlFor="filtro-prioridade" className="sr-only">Prioridade</label>
          <select 
            id="filtro-prioridade"
            value={filtroPrioridade}
            onChange={(e) => { setFiltroPrioridade(e.target.value); setPage(1); }} // Reseta a página ao mudar o filtro
            className="border-gray-300 rounded-md shadow-sm p-2 focus:ring-blue-500"
          >
        ...
        <div className="flex items-center">
          <input 
            ...
            onChange={(e) => { setHideCompleted(e.target.checked); setPage(1); }} // Reseta a página ao mudar o filtro
            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
          />
          <label htmlFor="hide-completed" className="ml-2 block text-sm text-gray-900">
            Ocultar concluídas/canceladas
          </label>
        </div>
        
        <button onClick={limparFiltros} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Limpar Filtros
        </button>
      </div>
          ...
          <>
                ...
                {ordens.length > 0 ? (
                  ordens.map((os) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-4">{os.equipamento?.codigo || <span className="text-sm text-red-600">Equipamento Deletado</span>}</td>
                      <td className="py-2 px-4">{os.solicitante?.nome || <span className="text-sm text-red-600">Usuário Deletado</span>}</td>
                      <td className="py-2 px-4 capitalize">{os.status.replace('_', ' ')}</td>
                      <td className="py-2 px-4 capitalize">{os.prioridade}</td>
                      <td className="py-2 px-4">{os.tecnicoResponsavel?.nome || <span className="text-gray-400">N/A</span>}</td>
            ...
            {ordens.length > 0 && totalPages > 1 && (
              <div className="mt-6 flex justify-between items-center">
                <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="px-4 py-2 bg-gray-300 rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
                  Anterior
                </button>
                <span>Página {page} de {totalPages}</span>
                <button onClick={() => handlePageChange(page + 1)} disabled={page === totalPages} className="px-4 py-2 bg-gray-300 rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
                  Próxima
                </button>
              </div>
            )}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Abrir Nova Ordem de Serviço">
        <OrdemDeServicoForm onFormSubmit={handleFormSubmit} onClose={() => setIsModalOpen(false)} />
      </Modal>
    </div>
  );
};
